...
Tactic Notation "si_cases" tactic(first) ident(c) :=
  first;
  [ Case_aux c "s_var1" | Case_aux c "s_var2"
  | Case_aux c "s_abs1" | Case_aux c "s_abs2"
  | Case_aux c "s_app" | Case_aux c "s_true" | Case_aux c "s_false"
  | Case_aux c "s_if" ].
...
Proof.
  ...
  Case "->".
    ...
    t_cases (induction t) SCase; intros t' x H; simpl in H; subst; auto.
    SCase "tvar".
      destruct (eq_id_dec x i); subst; auto.
    SCase "tabs".
      destruct (eq_id_dec x i); subst; auto.
  Case "<-".
    si_cases (induction H) SCase; subst; simpl; auto using eq_id.
    SCase "s_var2".
      destruct (eq_id_dec x y0).
      assert False; auto. solve by inversion.
      auto.
    SCase "s_abs1".
      rewrite eq_id. auto.
    SCase "s_abs2".
